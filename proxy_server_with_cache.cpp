...
int connectRemoteServer(char *remote_hostname, int remote_port)
{
	...
	memset((char *)&remote_server_addr, 0, sizeof(remote_server_addr)); // Clear structure
	...
	memcpy((char *)&remote_server_addr.sin_addr.s_addr, (char *)remote_host_info->h_addr, remote_host_info->h_length);
...
int handle_request(int client_socket_fd, ParsedRequest *parsed_http_request, char *original_request_string)
{
	...
	memset(http_request_buffer, 0, MAX_BYTES); // Clear buffer for receiving response
	...
	while (bytes_sent_to_server > 0)
	{
		...
		memset(http_request_buffer, 0, MAX_BYTES); // Clear buffer for next chunk
		bytes_sent_to_server = recv(remote_server_socket_fd, http_request_buffer, MAX_BYTES - 1, 0);
	}
...
void *thread_fn(void *client_socket_ptr)
{
	...
	memset(http_request_buffer, 0, MAX_BYTES);											 // Extra safety - clear buffer
	...
	for (int char_index = 0; char_index < (int)strlen(http_request_buffer); char_index++)
	{
	...
	if (cached_response != NULL)
	{
		...
		{
			memset(response_chunk_buffer, 0, MAX_BYTES);
	...
	else if (bytes_received_from_client > 0)
	{
		...
		{
			memset(http_request_buffer, 0, MAX_BYTES); // Clear buffer for reuse
...
int main(int argc, char *argv[])
{
	...
	memset((char *)&server_address_struct, 0, sizeof(server_address_struct)); // Clear structure
	...
	while (1)
	{
		...
		memset((char *)&client_address_struct, 0, sizeof(client_address_struct));
...
void remove_cache_element()
{
	...
	if (cache_head_pointer != NULL)
	{
		...
		total_cache_size_bytes = total_cache_size_bytes - (lru_element_to_remove->len) - sizeof(cache_element) - (int)strlen(lru_element_to_remove->url) - 1;
...
int add_cache_element(char *http_response_data, int response_data_size, char *cache_url_key)
{
	...
	int new_element_total_size = response_data_size + 1 + (int)strlen(cache_url_key) + (int)sizeof(cache_element);
